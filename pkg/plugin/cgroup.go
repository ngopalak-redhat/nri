...
// GetCgroupsV2AbsPath returns the absolute path to the cgroup v2 directory for a container.
...
func GetCgroupsV2AbsPath(container *api.Container) string {
...
// resolveCgroupPath resolves the cgroup path by trying different cgroup drivers
// It first detects if the path is systemd-style, then applies appropriate conversion
func resolveCgroupPath(cgroupRoot, cgroupPath string) string {
	// If it looks like a systemd path, convert it using systemd rules
	if isSystemdPath(cgroupPath) {
		systemdPath := convertSystemdPath(cgroupRoot, cgroupPath)
		// Try the systemd converted path first
		if pathExists(systemdPath) {
			return systemdPath
		}
		// If systemd path doesn't exist, still return it (may not exist yet)
		// This is because systemd paths should be converted regardless of existence
		return systemdPath
	}
